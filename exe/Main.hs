...
runIde dir Arguments{..} hiedb hiechan = do
    ...
    case argFilesOrCmd of
      ...
      Just argFiles -> do
        ...
        ide <- initialise def mainRule (pure $ IdInt 0) (showEvent lock) dummyWithProg (const (const id)) (logger logLevel) debouncer (defaultIdeOptions sessionLoader) vfs hiedb hiechan
