...
import Util
import Module (stringToUnitId, toInstalledUnitId, unitIdString)
...
lookupMod :: HieDb.HieDb -> FilePath -> ModuleName -> UnitId -> Bool -> MaybeT IdeAction Uri
lookupMod db hie_f mod uid boot
...
getAtPoint file pos = runMaybeT $ do
  ...
  AtPoint.atPoint opts hf hiedb pos'
...
getDefinition file pos = runMaybeT $ do
    ...
    let origFile
         ...
         | otherwise = Nothing
    AtPoint.gotoDefinition hiedb (lookupMod hiedb) opts hf origFile pos'
...
getTypeDefinition file pos = runMaybeT $ do
    ...
    let origFile
         | "cached-deps" `isInfixOf` (fromNormalizedFilePath file) = Just file
         | otherwise = Nothing
    AtPoint.gotoTypeDefinition hiedb (lookupMod hiedb) opts hf origFile pos'
...
getParsedModuleDefinition :: HscEnv -> IdeOptions -> [PackageName] -> NormalizedFilePath -> UTCTime -> Maybe T.Text -> IO (Maybe BS.ByteString, ([FileDiagnostic], Maybe ParsedModule))
    ...
    define $ \GetHieFile f -> do
      ...
      then liftIO $ do
        ...
        let res = case mres of
              Nothing -> Nothing
              Just res -> Just $ HFR res $ generateReferencesMap . getAsts . hie_asts $ res
        pure ([], res)
      else do
  ...
  addUsageDependencies $ liftIO $ do
    ...
    case res of
      (diags, Just (hsc,tcm))
        ...
        (diagsHie,hf) <- generateAndWriteHieFile hsc hiedbChan (tmrModule tcm) "" (mi_decl_docs $ hm_iface $ tmrModInfo tcm)
